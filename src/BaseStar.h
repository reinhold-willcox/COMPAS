...
public:
    ...
    virtual double          CalculateMomentOfInertiaAU(const double p_RemnantRadius = 0.0)  { return 0.0; }                                                                         // Use inheritance hierarchy
            ...
            double          CalculateRadialChange()                                                             { return (utils::Compare(m_RadiusPrev,0)<=0)? 0 : std::abs(m_Radius - m_RadiusPrev) / m_RadiusPrev; }                                      // Return fractional radial change (if previous radius is negative or zero, return 0 to avoid NaN
